...
const EditAccount = ({ openModal, setOpenModal }) => {
    ...
    const formattedAccountDate = accountDate ? accountDate.toISOString().split('T')[0] : "";
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.put('http://localhost:4000/customer/update/' + account._id, formData);
                ...
                <form>
                    ...
                    <label >
                        <input
                            ...
                            style={{ display: 'none' }}
                        />
                        <div style={{ position: 'absolute', top: 190, left: 320, width: '40px', height: '40px' }}>
                            <img src={require('../assets/camera.png')} alt="Tải lên avatar" style={{ width: '100%', height: '100%' }} />
                        </div>
                    </label>

                    <label >Họ và tên : </label>
                    <input type='text' value={name} placeholder={"Vui lòng nhập họ tên của bạn"} onChange={(e) => setName(e.target.value)} />
                    <label >Email :</label>
                    <input type='email' value={email} placeholder={"Vui lòng nhập địa chỉ email"} onChange={(e) => setEmail(e.target.value)} />
